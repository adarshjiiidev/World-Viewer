...
// configured via .env. Building on first request (like the GROQ_API_KEY reads) sees the loaded env;
// the result is cached so the limiter's per-IP window state persists. `null` = unlimited (default).
let _groqRateLimiter; // undefined = not built yet; null = unlimited; fn = active limiter
let _openAiRateLimiter; // lazy-initialization alias for backwards compat
let _googleRateLimiter;
/** Groq cost endpoints (chat completions + hud summary + /api/openai legacy path. Null = unlimited (default). */
function groqRateLimiter() {
  if (_groqRateLimiter === undefined) _groqRateLimiter = makeOptInRateLimiter(process.env.GEV_RATELIMIT_GROQ_PER_MIN || process.env.GEV_RATELIMIT_OPENAI_PER_MIN);
  return _groqRateLimiter;
}
/** Legacy alias for backwards compat with references inlined elsewhere. */
const openAiRateLimiter = groqRateLimiter;
...
// Primary AI backend: Groq (GROQ_API_KEY). OpenAI Realtime kept as fallback only
// when OPENAI_API_KEY is explicitly set (Groq does not yet offer Realtime WS).
const VOICE_PRIMARY_PROVIDER = 'groq';
const GROQ_HUD_MODEL_DEFAULT = 'groq/compound-mini';
const GROQ_REALTIME_CHAT_MODEL_DEFAULT = 'groq/compound';
...
const OPENAI_HUD_SUMMARY_MODEL_DEFAULT_LEGACY = 'gpt-5-nano';
...
/**
 * Vite plugin: Groq AI for HUD summaries + OpenAI Realtime ephemeral client secrets
 * (Realtime kept only as fallback: Groq does not currently offer Realtime WS API.)
 *
 * Keeps GROQ_API_KEY / OPENAI_API_KEY server-side. The browser only gets:
 *   - Short-lived Groq chat responses (HUD five-word summaries), or
 *   - A short-lived OpenAI Realtime client secret for WebRTC voice.
 */
function openAiRealtimeProxy() {
  function install(middlewares) {
    // ==== HUD FIVE-WORD SUMMARY (Groq chat/completions primary) ==============
    async function handleHudSummary(req, res) {
      ...
      if (!enforceOptInRateLimit(groqRateLimiter(), req, res)) return;

      const apiKey = process.env.GROQ_API_KEY || process.env.OPENAI_API_KEY;
      const preferGroq = Boolean(process.env.GROQ_API_KEY);
      if (!apiKey) {
        res.statusCode = 503;
        res.setHeader('Content-Type', 'application/json');
      ...
      try {
        ...
        if (preferGroq) {
          const model = process.env.GROQ_HUD_MODEL || GROQ_HUD_MODEL_DEFAULT;
          const upstream = await fetch(`${GROQ_API_BASE}/chat/completions`, {
            ...
            body: JSON.stringify({
              ...
              max_tokens: Math.min(100, 2048),
              temperature: 0.3,
            }),
            signal: AbortSignal.timeout(20000),
          });
          const text = await readResponseTextCapped(upstream, GROQ_MAX_RESPONSE_BYTES);
          let data;
          try { data = JSON.parse(text); } catch { data = {}; }
          summary = toFiveWordHudSummary(extractGroqChatText(data));
          httpStatus = upstream.status;
          if (!upstream.ok) errMsg = data.error?.message || 'Groq HUD summary request failed';
        } else {
          // Fallback to OpenAI Responses API when only OPENAI_API_KEY is set
          const response = await fetch('https://api.openai.com/v1/responses', {
            ...
            body: JSON.stringify({
              model: process.env.OPENAI_HUD_SUMMARY_MODEL || OPENAI_HUD_SUMMARY_MODEL_DEFAULT_LEGACY,
              instructions,
          ...
          if (!response.ok) errMsg = data.error?.message || 'OpenAI HUD summary request failed';
        }

        res.statusCode = summary ? 200 : httpStatus;
        res.setHeader('Content-Type', 'application/json; charset=utf-8');
        res.setHeader('Cache-Control', 'no-store');
        res.end(JSON.stringify({
          summary: summary || null,
          error: summary ? null : (errMsg || 'HUD summary request failed'),
          provider: preferGroq ? 'groq' : 'openai-fallback',
        }));
      } catch (error) {
        res.statusCode = 502;
        res.setHeader('Content-Type', 'application/json');
    ...
    // Route: Groq-native path (primary, what hud.js now calls)
    middlewares.use('/api/groq/hud-summary', handleHudSummary);
    // Route: Legacy /api/openai/hud-summary — same handler (for anyone still calling it)
    ...
    middlewares.use('/api/realtime/token', async (req, res) => {
      ...
      // Opt-in per-IP throttle (GEV_RATELIMIT_GROQ_PER_MIN, with GEV_RATELIMIT_OPENAI_PER_MIN legacy alias). No-op when unset.
      if (!enforceOptInRateLimit(groqRateLimiter(), req, res)) return;

      // Groq does not yet offer a Realtime WebSocket / client_secrets API.
      // Voice sessions therefore rely on OpenAI's Realtime endpoint as the
      // operational backend. To keep naming consistent with the Groq-first
      // policy, accept the key through either env var — the preferred new
      // GROQ_REALTIME_OPENAI_API_KEY or the legacy OPENAI_API_KEY.
      const apiKey =
        process.env.GROQ_REALTIME_OPENAI_API_KEY ||
        process.env.OPENAI_API_KEY;
      if (!apiKey) {
        res.statusCode = 503;
        res.setHeader('Content-Type', 'application/json');
        res.end(JSON.stringify({ error: 'GROQ_REALTIME_OPENAI_API_KEY is not set (voice/realtime requires an OpenAI Realtime-compatible key)' }));
        return;
      }
      ...
      const envFor = (legacy, groqKey, fallback) =>
        process.env[groqKey] || process.env[legacy] || fallback;
      const model =
        tier === 'mini'
          ? envFor('OPENAI_REALTIME_MODEL_MINI', 'GROQ_REALTIME_MODEL_MINI', OPENAI_REALTIME_MODEL_MINI_DEFAULT)
          : envFor('OPENAI_REALTIME_MODEL', 'GROQ_REALTIME_MODEL', OPENAI_REALTIME_MODEL_DEFAULT);
      const voice = envFor('OPENAI_REALTIME_VOICE', 'GROQ_REALTIME_VOICE', OPENAI_REALTIME_VOICE_DEFAULT);
      const effort = envFor('OPENAI_REALTIME_REASONING_EFFORT', 'GROQ_REALTIME_REASONING_EFFORT', OPENAI_REALTIME_REASONING_DEFAULT);
      const contextTokenLimit = Math.round(Math.max(
        1000,
        Math.min(12000, Number(envFor('OPENAI_REALTIME_CONTEXT_TOKENS', 'GROQ_REALTIME_CONTEXT_TOKENS', String(OPENAI_REALTIME_CONTEXT_TOKENS_DEFAULT))) || OPENAI_REALTIME_CONTEXT_TOKENS_DEFAULT)
      ));
      const contextRetentionRatio = Math.max(
        0.1,
        Math.min(1, Number(envFor('OPENAI_REALTIME_CONTEXT_RETENTION', 'GROQ_REALTIME_CONTEXT_RETENTION', String(OPENAI_REALTIME_CONTEXT_RETENTION_DEFAULT))) || OPENAI_REALTIME_CONTEXT_RETENTION_DEFAULT)
      );
      const sessionConfig = {
        session: {
          type: 'realtime',
...
function extractGroqChatText(data) {
  // Standard OpenAI-compatible /chat/completions response: choices[0].message.content
...
// ─────────────────────────────────────────────────────────────────────────────
// WORLD VIEWER — Groq AI Proxy
// Keeps GROQ_API_KEY server-side. Exposes chat, health, transcription and
// speech endpoints; browser clients never receive the credential.
// ─────────────────────────────────────────────────────────────────────────────
const GROQ_API_BASE = 'https://api.groq.com/openai/v1';
const GROQ_MAX_BODY_BYTES = 16 * 1024;   // 16 KB — context + question
const GROQ_MAX_RESPONSE_BYTES = 4 * 1024 * 1024;  // 4 MB — generous for streaming

function groqProxy() {
  function install(middlewares) {
    // Health check — lets worldViewer.js know if Groq is configured
    middlewares.use('/api/groq/health', (req, res) => {
      const key = process.env.GROQ_API_KEY;
      res.statusCode = key ? 200 : 503;
      res.setHeader('Content-Type', 'application/json');
      res.end(JSON.stringify({ ok: Boolean(key), model: process.env.GROQ_HUD_MODEL || 'llama-3.1-8b-instant' }));
    });

    // Chat completions proxy
    middlewares.use('/api/groq/chat', async (req, res) => {
      if (req.method !== 'POST') {
      ...
      if (!apiKey) {
        ...
        res.end(JSON.stringify({ error: 'GROQ_API_KEY is not set' })); return;
      }
      try {
        const rawBody = await readRequestBodyCapped(req, GROQ_MAX_BODY_BYTES);
        const body = JSON.parse(rawBody.toString());
        // Enforce safe model selection — allow only accessible Groq models
        const allowedModels = [
          // Standard Groq compound models
          'groq/compound', 'groq/compound-mini',
          // Qwen models
          'qwen/qwen3.8-27b', 'qwen/qwen3.6-27b',
          // OpenAI via Groq
          'openai/gpt-oss-120b', 'openai/gpt-oss-20b',
          // Legacy names (keep for compat if key changes)
          'llama-3.3-70b-versatile', 'llama-3.1-8b-instant',
          'llama-3.1-70b-versatile', 'mixtral-8x7b-32768', 'gemma2-9b-it',
        ];
        const defaultModel = process.env.GROQ_HUD_MODEL || 'groq/compound-mini';
        const model = allowedModels.includes(body.model) ? body.model : defaultModel;
        ...
        res.statusCode = 502;
        res.setHeader('Content-Type', 'application/json');
        res.end(JSON.stringify({ error: err?.message || 'Groq upstream error' }));
      }
    });

    middlewares.use('/api/groq/transcriptions', async (req, res) => {
      if (req.method !== 'POST') {
        res.statusCode = 405; res.end(JSON.stringify({ error: 'Method not allowed' })); return;
      }
      const apiKey = process.env.GROQ_API_KEY;
      ...
      try {
        const audio = await readRequestBodyCapped(req, 25 * 1024 * 1024);
        const upstream = await fetch(`${GROQ_API_BASE}/audio/transcriptions`, {
          method: 'POST',
          headers: { Authorization: `Bearer ${apiKey}`, 'Content-Type': req.headers['content-type'] || 'application/octet-stream' },
          body: audio,
        ...
        res.end(JSON.stringify({ error: error?.message || 'Groq transcription request failed' }));
      }
    });

    middlewares.use('/api/groq/speech', async (req, res) => {
      if (req.method !== 'POST') {
        res.statusCode = 405; res.end(JSON.stringify({ error: 'Method not allowed' })); return;
      }
      const apiKey = process.env.GROQ_API_KEY;
      ...
      try {
        const body = JSON.parse((await readRequestBodyCapped(req, 8 * 1024)).toString() || '{}');
        const input = String(body.input || '').trim().slice(0, 500);
        if (!input) throw new Error('Speech input is required');
        const upstream = await fetch(`${GROQ_API_BASE}/audio/speech`, {
          method: 'POST',
          headers: { Authorization: `Bearer ${apiKey}`, 'Content-Type': 'application/json' },
          body: JSON.stringify({ model: 'canopylabs/orpheus-v1-english', voice: 'hannah', input, response_format: 'wav' }),
          signal: AbortSignal.timeout(30_000),
        });
        ...
        res.statusCode = 502; res.setHeader('Content-Type', 'application/json');
        res.end(JSON.stringify({ error: error?.message || 'Groq speech request failed' }));
      }
    });
  }
  return { name: 'world-viewer-groq-proxy', configureServer(s) { install(s.middlewares); } };
}

// ─────────────────────────────────────────────────────────────────────────────
// WORLD VIEWER — reviewed World Monitor RSS bridge
// Browser feeds often omit CORS headers. This narrow feed-id bridge mirrors
// the packaged Electron broker and deliberately never accepts arbitrary URLs.
// ─────────────────────────────────────────────────────────────────────────────
const WORLD_VIEWER_NEWS_FEEDS = Object.freeze({
...
const _worldViewerNewsCache = new Map();

function worldViewerNewsProxy() {
  function install(middlewares) {
    middlewares.use('/api/world-news', async (req, res) => {
      ...
      if (search) {
        ...
        const query = search.replace(/[<>]/g, ' ').replace(/\s+/g, ' ').trim();
        const key = `search:${query.toLowerCase()}`;
        const cachedSearch = _worldViewerNewsCache.get(key);
        if (cachedSearch && Date.now() - cachedSearch.at < 90_000) {
          res.writeHead(200, { 'Content-Type': cachedSearch.contentType, 'Cache-Control': 'no-store', 'X-World-Viewer-Cache': 'HIT' });
          res.end(cachedSearch.body);
          return;
        }
        try {
          const upstream = await fetch(`https://news.google.com/rss/search?q=${encodeURIComponent(`${query} when:7d`)}&hl=en-US&gl=US&ceid=US:en`, {
          ...
          _worldViewerNewsCache.set(key, { at: Date.now(), body, contentType });
          res.writeHead(200, { 'Content-Type': contentType, 'Cache-Control': 'no-store', 'X-World-Viewer-Cache': 'MISS' });
          res.end(body);
        } catch (error) {
          res.writeHead(502, { 'Content-Type': 'application/json' });
          res.end(JSON.stringify({ error: `News search unavailable: ${error.message}` }));
        }
        return;
      }
      const feedId = params.get('feed') || '';
      const upstreamUrl = WORLD_VIEWER_NEWS_FEEDS[feedId];
      if (!upstreamUrl) {
        res.writeHead(400, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify({ error: 'Unknown reviewed news feed' }));
        return;
      }
      const cached = _worldViewerNewsCache.get(feedId);
      if (cached && Date.now() - cached.at < 90_000) {
        res.writeHead(200, { 'Content-Type': cached.contentType, 'Cache-Control': 'no-store', 'X-World-Viewer-Cache': 'HIT' });
        res.end(cached.body);
        return;
      }
      try {
        ...
        if (!body || body.length > 2 * 1024 * 1024) throw new Error('Feed response is empty or exceeds 2 MB');
        const contentType = upstream.headers.get('content-type') || 'application/xml; charset=utf-8';
        _worldViewerNewsCache.set(feedId, { at: Date.now(), body, contentType });
        res.writeHead(200, { 'Content-Type': contentType, 'Cache-Control': 'no-store', 'X-World-Viewer-Cache': 'MISS' });
        res.end(body);
      } catch (error) {
        if (cached) {
          res.writeHead(200, { 'Content-Type': cached.contentType, 'Cache-Control': 'no-store', 'X-World-Viewer-Cache': 'STALE' });
          res.end(cached.body);
          return;
        }
        res.writeHead(502, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify({ error: `News feed unavailable: ${error.message}` }));
      }
    });
  }
  return {
    name: 'world-viewer-news-proxy',
    configureServer(server) { install(server.middlewares); },
    configurePreviewServer(server) { install(server.middlewares); },
  };
}

function placeContextProxy() {
  async function handler(req, res) {
    ...
    try {
      ...
      res.writeHead(200, { 'Content-Type': 'application/json', 'Cache-Control': 'no-store' });
      res.end(JSON.stringify({ label, lat, lon, source: 'OpenStreetMap Nominatim', freshness: 'recent' }));
    } catch (error) {
      res.writeHead(502, { 'Content-Type': 'application/json' });
      res.end(JSON.stringify({ error: error?.message || 'Reverse geocoding unavailable' }));
    }
  }
  return { name: 'world-viewer-place-context-proxy', configureServer(server) { server.middlewares.use('/api/place-context', handler); }, configurePreviewServer(server) { server.middlewares.use('/api/place-context', handler); } };
}

// ─────────────────────────────────────────────────────────────────────────────
// WORLD VIEWER — WorldMonitor conflict data proxy (war hotspots)
// Bridges gods-eye-view-main -> worldmonitor-main conflict APIs so the
// hotspot engine can use real UCDP armed-conflict events instead of
// AI-synthetic data.
//
// Priority chain for data:
//   1. LIVE upstream via WORLD_MONITOR_BASE_URL (default: https://api.worldmonitor.app)
//      /api/conflict/v1/list-ucdp-events?country=...
//   2. Local filesystem read from the sibling worldmonitor-main checkout at
//      `./sources/worldmonitor-main` — looks for a seeded/exported
//      `conflict-ucdp-events-v1.json` file in its data/ directory.
//   3. Empty result.
//
// Exposes: GET /api/world-monitor/conflict/ucdp-events
// Optional query params: ?country=UA ?limit=200
// ─────────────────────────────────────────────────────────────────────────────
const WORLDMONITOR_DEFAULT_BASE_URL = 'https://api.worldmonitor.app';
const WORLDMONITOR_UCDP_CACHE_MS = 15 * 60 * 1000; // 15 min in-memory cache
let _worldmonitorUcdpCache = null;
let _worldmonitorUcdpCacheTs = 0;
function worldMonitorConflictProxy() {
  function install(middlewares) {
    middlewares.use('/api/world-monitor/conflict/ucdp-events', async (req, res) => {
      if (req.method !== 'GET') {
        res.statusCode = 405;
        res.setHeader('Content-Type', 'application/json');
        res.end(JSON.stringify({ events: [], error: 'Method not allowed' }));
        return;
      }
      try {
        const now = Date.now();
        const requestUrl = new URL(req.url || '', 'http://localhost');
        const country = (requestUrl.searchParams.get('country') || '').toUpperCase().slice(0, 4);
        const limit = Math.min(500, Number(requestUrl.searchParams.get('limit')) || 300);
        const forceRefresh = requestUrl.searchParams.has('refresh');

        const fromCache = () => {
          if (!_worldmonitorUcdpCache || (now - _worldmonitorUcdpCacheTs) > WORLDMONITOR_UCDP_CACHE_MS) return null;
          if (!_worldmonitorUcdpCache) return null;
          return _worldmonitorUcdpCache;
        };
        const writeCache = (value) => {
          _worldmonitorUcdpCache = value;
          _worldmonitorUcdpCacheTs = now;
        };

        let eventsArr = null;
        if (!forceRefresh) {
          const cached = fromCache();
          if (cached && Array.isArray(cached)) eventsArr = cached;
        }

        // Fetch from upstream or local checkout if cache miss
        if (eventsArr === null) {
          const base = (process.env.WORLD_MONITOR_BASE_URL || WORLDMONITOR_DEFAULT_BASE_URL).replace(/\/$/, '');
          let fetched = null;
          try {
            const upstream = await fetch(
              `${base}/api/conflict/v1/list-ucdp-events${country ? `?country=${encodeURIComponent(country)}` : ''}`,
              {
                method: 'GET',
                headers: {
                  'Accept': 'application/json',
                  'User-Agent': 'gods-eye-view/world-viewer-hotspots (+https://github.com/gods-eye-view/world-viewer)',
                },
            ...
            if (upstream.ok) {
              ...
              fetched = Array.isArray(json?.events) ? json.events : (Array.isArray(json) ? json : null);
            }
          } catch (_err) { /* upstream unavailable -> try local checkout */ }

          // Local checkout fallback: read <repo-root>/sources/worldmonitor-main/data/conflict-ucdp-events-v1.json
          if (!Array.isArray(fetched) || fetched.length === 0) {
            try {
              const fs = await import('node:fs');
              const path = await import('node:path');
              const rootDir = __dirname;
              const candidate = path.resolve(rootDir, '..', '..', '..', 'sources', 'worldmonitor-main', 'data', 'conflict-ucdp-events-v1.json');
              try {
              ...
              } catch (_fsErr) { /* ignore file-read errors */ }
            } catch (_noFs) { /* ignore */ }
          }

          if (Array.isArray(fetched)) {
            eventsArr = fetched;
            writeCache(eventsArr);
          } else {
            eventsArr = [];
            writeCache([]);
          }
        ...
        let events = Array.isArray(eventsArr) ? eventsArr : [];
        if (country) events = events.filter((e) => (e.country || e.isoCode || e.ISO || '').toUpperCase() === country);
        events = events.slice(0, limit);

        res.statusCode = 200;
        res.setHeader('Content-Type', 'application/json; charset=utf-8');
        res.setHeader('Cache-Control', `public, max-age=${Math.floor(WORLDMONITOR_UCDP_CACHE_MS / 1000)}, stale-while-revalidate=300`);
        res.end(JSON.stringify({ events, count: events.length, source: 'worldmonitor-ucdp' }));
      } catch (err) {
        res.statusCode = 502;
        res.setHeader('Content-Type', 'application/json');
        res.end(JSON.stringify({ events: [], error: err?.message || 'Failed to load WorldMonitor conflict data' }));
      }
    });
  }
  return {
    name: 'world-monitor-conflict-proxy',
...
export default defineConfig(({ mode }) => {
  ...
  return {
    plugins: [
      ...
      openAiRealtimeProxy(),
      groqProxy(),
      worldViewerNewsProxy(),
      placeContextProxy(),
      worldMonitorConflictProxy(),
    ...
    define: {
      ...
      'import.meta.env.AISSTREAM_API_KEY': JSON.stringify(env.AISSTREAM_API_KEY),
      'import.meta.env.TOMTOM_API_KEY': JSON.stringify(env.TOMTOM_API_KEY),
