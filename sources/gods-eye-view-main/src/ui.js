...
export class StyleManager {
  ...
  _initUI() {
    ...
    // Keyboard shortcuts: 1-7, H, Escape, Ctrl+K, L/T/E/P
    this._globalKeydownHandler = (e) => {
      // Ctrl+K / Cmd+K — universal command palette (works even inside inputs).
      if ((e.ctrlKey || e.metaKey) && !e.shiftKey && !e.altKey && e.key?.toLowerCase() === 'k') {
        e.preventDefault();
        const cp = window.__godsEyeView?.commandPalette;
        if (cp && typeof cp.show === 'function') cp.show();
        return;
      }

      // Ignore when interacting with a form control (except Escape). Global
      // hotkeys ('1'-'7', 'h', 'o', 'v', 'd', 'c', 'f', 'g', 'l', 't', 'e',
      // 'p') otherwise fire while a <select> dropdown (e.g. HUD layout) is
      ...
      if (e.key.toLowerCase() === 'g') {
        this.shareLinkManager?.claimRestoreLane?.('panel', 'global-context-panel');
        const panelEl = document.getElementById('global-context-panel');
        if (panelEl) {
          const collapsed = panelEl.classList.contains('collapsed');
          this.setPanelCollapsed('global-context-panel', !collapsed, {
            explicit: true,
            persist: true,
            syncShare: true,
          });
      ...
      if (e.key.toLowerCase() === 'l') {
        const ls = window.__godsEyeView?.layerSelector;
        if (ls && typeof ls.toggle === 'function') ls.toggle();
      }
      if (e.key.toLowerCase() === 't') {
        const tm = window.__godsEyeView?.timeline;
        if (tm && typeof tm.toggle === 'function') {
          tm.toggle();
        } else if (typeof tm?.show === 'function' && typeof tm?.hide === 'function') {
          const bar = document.getElementById('wv-timeline');
      ...
      if (e.key.toLowerCase() === 'e') {
        const ep = window.__godsEyeView?.eventsPanel;
        if (ep && typeof ep.toggle === 'function') ep.toggle();
      }
      if (e.key.toLowerCase() === 'p') {
        const wp = window.__godsEyeView?.worldPulse;
        if (wp) {
          const panel = document.getElementById('wv-world-pulse');
          if (!panel) return;
          if (panel.hidden || panel.classList.contains('hidden')) {
            if (typeof wp.show === 'function') wp.show();
          } else {
            if (typeof wp.hide === 'function') wp.hide();
          }
        }
      }
      if (e.key === ' ' && !e.shiftKey && !e.metaKey && !e.ctrlKey && !e.altKey) {
        const director = window.__godsEyeView?.sceneDirector;
        if (director && typeof director.stopScene === 'function' && typeof director.startScene === 'function') {
          e.preventDefault();
          if (director._running) {
            director.stopScene('Toggled (Space)');
          } else {
            void director.startScene(director._selectedSceneId, { single: false });
          }
        }
      }
      if (e.key.toLowerCase() === 'r') {
        this.shareLinkManager?.claimRestoreLane?.('navigation');
