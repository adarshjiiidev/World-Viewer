...
import { initGroqPushToTalk } from './groqPushToTalk.js';
import {
  DEFAULT_VOICE_TIER,
  VOICE_COST_LIMITS,
...
export function initGevVoiceCommands({ viewer, styleManager, dataManager, sceneDirector = null, annotations = null }) {
  ...
  // Groq supports speech-to-text and text-to-speech but not OpenAI's WebRTC
  // Realtime client-secret protocol. Offer its documented press-and-hold path
  // alongside the optional OpenAI live-session control.
  controller.groqPushToTalk = initGroqPushToTalk({ ui });
...
export class GevRealtimeController {
  ...
  async start({ pushToTalk = false } = {}) {
    ...
    try {
      ...
      this.stop({ preserveStatus: true });
      // This control uses OpenAI's WebRTC Realtime protocol by design. A
      // configured Groq key cannot mint that protocol's client secret, so this
      // is a capability choice rather than a broken Groq integration. Keep the
      // UI actionable instead of raising a misleading system-error panel.
      if (/GROQ_REALTIME_OPENAI_API_KEY|OpenAI Realtime-compatible key/i.test(error?.message || '')) {
        this.setStatus('idle', 'LIVE VOICE NEEDS OPENAI · USE HOLD GROQ');
        return;
      }
  ...
  updateVoiceButtonLabel() {
    ...
    this.ui.buttonLabel.textContent = 'LIVE';
    if (this.ui.helpDetail) {
      this.ui.helpDetail.textContent = 'LIVE requires OpenAI Realtime · hold GROQ for Groq voice';
    }
  }
...
function createVoiceControl({ reset = false } = {}) {
  ...
  if (!root) {
    ...
    root.innerHTML = `
      ...
      <button id="gev-voice-button" type="button" aria-label="Voice control — hold Space to speak; click to toggle voice" aria-describedby="gev-voice-help">
        ...
        <span class="gev-mic-label">LIVE</span>
      </button>
      <button id="gev-groq-ptt" type="button" title="Hold to talk using Groq speech-to-text and text-to-speech">HOLD · GROQ</button>
  ...
  return {
    ...
    groqPttButton: root.querySelector('#gev-groq-ptt'),
