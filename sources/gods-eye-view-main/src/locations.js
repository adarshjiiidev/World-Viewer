...
function navigationCallbacks(options = {}) {
  return {
    onStart: options.onStart,
    onComplete: options.onComplete,
    onCancel: options.onCancel,
...
export async function searchAndFlyTo(viewer, query, options = {}) {
  ...
  // A key should improve landmark precision, not be a prerequisite for basic
  // world exploration. Open-Meteo's public place search is used only as a
  // clearly limited city/region fallback; it never claims building precision.
  if (!apiKey) {
    const response = await fetch(`https://geocoding-api.open-meteo.com/v1/search?name=${encodeURIComponent(query)}&count=1&language=en&format=json`);
    const fallback = (await response.json().catch(() => null))?.results?.[0];
    if (!fallback || !Number.isFinite(fallback.latitude) || !Number.isFinite(fallback.longitude)) return null;
    if (!mayFly()) return CANCELLED_SEARCH;
    const range = finitePositive(options.range) || 180_000;
    flyToLandmark(viewer, fallback.latitude, fallback.longitude, {
      range,
      pitch: 42,
      heading: 0,
      buildingHeight: 0,
      duration: finitePositive(options.duration) || 3,
      ...navigationCallbacks(options),
    });
    return {
      label: [fallback.name, fallback.admin1, fallback.country].filter(Boolean).join(', '),
      navigationMode: 'public-place-fallback',
      rangeM: range,
      freshness: 'estimated',
    };
  }
